    ...
    for fold in [1, 2, 3]:
        ...
        for i in range(1, 61):
            columns.append(pd.DataFrame({
                'x1_' + str(i): predictions[:, i - 1, 0],
                'y1_' + str(i): predictions[:, i - 1, 1],
                'x2_' + str(i): predictions[:, i - 1, 2],
                'y2_' + str(i): predictions[:, i - 1, 3]
            }))
            
        results_df = pd.concat([results_df] + columns, axis=1)
