...
if __name__ == '__main__':
    ...
    for detector in ['mask-rcnn']:
        for fold in [1]:
            encoder = models.EncoderRNN(device, num_hidden, layers_enc).float().to(device)
            decoder = models.DecoderRNN(device, num_hidden, dropout_p, layers_dec).float().to(device)
